package tec.inf.javaEE.lab2023.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "javaEE.lab2023-ear.ear");

        // Current directory is the root of the "javaEE.lab2023-web" project. Go up one level
        // and enter the "javaEE.lab2023-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../javaEE.lab2023-ejb/target/javaEE.lab2023-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "javaEE.lab2023-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/javaEE.lab2023-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "javaEE.lab2023-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("tec.inf.javaEE.lab2023.test");
